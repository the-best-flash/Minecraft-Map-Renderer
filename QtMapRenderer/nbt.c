...
int nbt_read(nbt_file *nbt, nbt_type type, void **parent)
{
    ...
    switch (type)
    {
        ...
        case TAG_BYTE_ARRAY:
            ;; /* ... */
            len = nbt_read_byte_array(nbt, &bytestring);
            
            t = (nbt_byte_array*)malloc(sizeof(nbt_byte_array));
            t->length = len;
        ...
        case TAG_LIST:
            ;; 
            length = nbt_read_list(nbt, &type_c, &target);

            l = (nbt_list*)malloc(sizeof(nbt_list));
            l->length = length;
            l->type = (nbt_type)type_c;
            l->content = target;
        ...
        case TAG_COMPOUND:
            ;;
            c = (nbt_compound*)malloc(sizeof(nbt_compound));

            lc = nbt_read_compound(nbt, &tags);
...
int32_t nbt_read_compound(nbt_file *nbt, nbt_tag ***listptr)
{
    ...
    *listptr = (nbt_tag**)malloc(sizeof(nbt_tag *)); 
    ...
    for (i = 0;; ++i)
    {
        ...
        last = nbt_read_tag(nbt, &((*listptr)[i]));
